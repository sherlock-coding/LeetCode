...
55 Jump Game
判断能否到达数组中最后一个数。
从后向前，判断每个位置是否能到达最后，并保存最小的能到达最后的那个位置。
...
得出n个数的排列中的第k个。
构造进位分别n,...,1的数。给出k，得到k-1对应的数，如2001，表示首先从1,2,3,4中取第二个，再从剩下来取第0个。。。

2015.12.20
61 Rotate List
向右旋转链表
需要计算链表的长度，来确定真正需要右旋的长度。
...
78 Subsets
子集
1、基于DFS的递归。每个元素有两种状态，要么选择，要么不选择。
2、基于同质的递归。比原来问题小却同质的问题。如{1,2，3}的子集，可以先求{2,3}的子集。{2,3}的子集同样时{1,2,3}的子集，再将[2,3]的子集加上1后就得到所有{1,2,3}的子集。
