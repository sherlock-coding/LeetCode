...
{
    vector<int> answer;
    // pair.sencond, 0表示左子树未访问， 1表示左子树已访问, 2表示右子树已访问
...
int main()
{
    ...
    vector<int> answer = postorderTraversalRecursively(root);
    vector<int> answer1 = postorderTraversalIteratively(root);
    for (auto i : answer)
        cout << i << " ";
    cout << endl;
    for (auto i : answer1)
